...
`electron/tsconfig.json` sets `noUnusedLocals: false` and `noUnusedParameters: false` so that
...
```
npx tsc -p electron/tsconfig.json --noEmit --noUnusedLocals --noUnusedParameters
```
...
`electron/tsconfig.json` and delete this file.
