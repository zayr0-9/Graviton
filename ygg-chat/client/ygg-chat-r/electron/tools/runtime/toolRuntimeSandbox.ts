// electron/tools/runtime/toolRuntimeSandbox.ts
// Runtime-neutral host for the out-of-process tool sandbox.
//
// This class owns the full request/response lifecycle against
// toolRuntimeUtility.mjs: ready handshake, per-request timeouts, telemetry,
// and shutdown. It does not know how the child process is created — the host
// supplies a fork function:
//   - Electron: utilityProcess.fork (see UtilityToolRuntimeHost.ts)
//   - standalone Node: child_process.fork (see nodeToolRuntimeHost.ts)
// The message protocol (protocol.ts) is identical on both hosts.
//
...
/**
 * Minimal surface the sandbox host needs from a child process. Both
 * `Electron.UtilityProcess` and Node `ChildProcess` adapt onto this.
 */
...
export type SandboxProcessForker = (entryPath: string) => SandboxProcessHandle

export interface ToolRuntimeSandboxOptions {
  fork: SandboxProcessForker
  /** Path to the built toolRuntimeUtility.mjs. Defaults to a sibling of this bundle. */
  entryPath?: string
}
...
function defaultUtilityEntryPath(): string {
  // When bundled, import.meta.url collapses to the bundle location, so this
  // resolves to a toolRuntimeUtility.mjs sitting next to main.mjs (Electron)
  // or next to the standalone server bundle.
...
export class ToolRuntimeSandboxHost {
  private readonly forkProcess: SandboxProcessForker
  private readonly entryPath: string
  private process: SandboxProcessHandle | null = null
  ...
  constructor(options: ToolRuntimeSandboxOptions) {
    this.forkProcess = options.fork
    this.entryPath = options.entryPath ?? defaultUtilityEntryPath()
  }
  ...
  async initialize(): Promise<void> {
    ...
    this.process = this.forkProcess(this.entryPath)
    ...
    this.process.onMessage((rawMessage: unknown) => {
    ...
    this.process.onExit((code: number | null) => {
    ...
    this.process.onError((error: Error) => {
