...
describe('resolveGatewayFlags', () => {
  ...
  it('an explicit gateway.chat === false Conf key forces chat off (the escape hatch)', () => {
    ...
    expect(resolveGatewayFlags()).toEqual({ chat: false, tokenOwner: false, crud: false, cloudProxy: false, resumableRuns: false })
  })
  ...
  it('chat stays on for any non-false chat value; other flags need === true', () => {
    ...
    expect(resolveGatewayFlags()).toEqual({ chat: true, tokenOwner: true, crud: false, cloudProxy: false, resumableRuns: false })
  ...
  it('reads the Phase 5 crud + cloudProxy keys independently (only === true enables)', () => {
    ...
    expect(resolveGatewayFlags()).toEqual({ chat: true, tokenOwner: false, crud: true, cloudProxy: false, resumableRuns: false })

    mockConf.get = key => (key === 'gateway.cloudProxy' ? true : undefined)
    expect(resolveGatewayFlags()).toEqual({ chat: true, tokenOwner: false, crud: false, cloudProxy: true, resumableRuns: false })

    mockConf.get = key => (key === 'gateway.crud' || key === 'gateway.cloudProxy' ? true : 0)
    // chat: 0 !== false => stays on.
    expect(resolveGatewayFlags()).toEqual({ chat: true, tokenOwner: false, crud: true, cloudProxy: true, resumableRuns: false })
  })

  it('reads gateway.resumableRuns independently, default OFF (only === true enables)', () => {
    mockConf.get = key => (key === 'gateway.resumableRuns' ? true : undefined)
    expect(resolveGatewayFlags()).toEqual({ chat: true, tokenOwner: false, crud: false, cloudProxy: false, resumableRuns: true })

    // A non-boolean truthy value must NOT enable it (=== true gate).
