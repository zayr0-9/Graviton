...
describe('resolveGatewayFlags', () => {
  ...
  it('defaults chat and resumable runs ON with no env override and no Conf keys set', () => {
    ...
    expect(resolveGatewayFlags()).toEqual({ chat: true, crud: false, cloudProxy: false, resumableRuns: true })
  })
  ...
  it('YGG_GATEWAY_MODE truthy is a master override that turns every flag on', () => {
    for (const truthy of ['1', 'true', 'yes', 'on', 'TRUE']) {
      ...
      expect(resolveGatewayFlags()).toEqual({ chat: true, crud: true, cloudProxy: true, resumableRuns: true })
    }
  })
  ...
  it('an explicit gateway.chat === false Conf key forces chat off (the escape hatch)', () => {
    ...
    expect(resolveGatewayFlags()).toEqual({ chat: false, crud: false, cloudProxy: false, resumableRuns: true })
  })
  ...
  it('reads the Phase 5 crud + cloudProxy keys independently (only === true enables)', () => {
    ...
    expect(resolveGatewayFlags()).toEqual({ chat: true, crud: true, cloudProxy: false, resumableRuns: true })

    mockConf.get = key => (key === 'gateway.cloudProxy' ? true : undefined)
    expect(resolveGatewayFlags()).toEqual({ chat: true, crud: false, cloudProxy: true, resumableRuns: true })
  ...
  it('reads gateway.resumableRuns independently and only explicit false opts out', () => {
    ...
    expect(resolveGatewayFlags()).toEqual({ chat: true, crud: false, cloudProxy: false, resumableRuns: false })
