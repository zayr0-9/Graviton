...
const loadTestHarnessHtml = (): string => {
  ...
  try {
    // dist-server layout: page copied next to ygg-server.mjs
  ...
  try {
    // packaged Electron layout: bundle at electron/main.mjs, page under server/
    candidatePaths.push(fileURLToPath(new URL('../server/headlessServer/routes/testHarnessPage.html', import.meta.url)))
  } catch {
    // Ignore malformed URL conversion and try additional locations.
  }

  candidatePaths.push(
    path.resolve(process.cwd(), 'server', 'headlessServer', 'routes', 'testHarnessPage.html'),
