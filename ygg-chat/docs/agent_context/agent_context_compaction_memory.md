...
- Compaction knobs travel on the request contract (`electron/headlessServer/contracts/headlessApi.ts`: `autoCompactionEnabled`, `contextLength`, `compactionThresholdPercent`, `compactionProvider`, `compactionModelName`, `compactionSystemPrompt`) and are threaded through `ChatOrchestrator` into `ToolLoopRunInput`. The renderer forwards its auto-compaction setting plus the selected model's context length; callers that omit a field (mobile/direct callers) retain server defaults (`autoCompactionEnabled ?? true`, `contextLength ?? openAIModelContextLength(modelName)`, threshold default 85, compaction provider/model fall back to the turn's provider/model).
